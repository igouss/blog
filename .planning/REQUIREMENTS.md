...
- [ ] **INFRA-01**: Blog files are organized in a structured directory layout (`articles/`, `templates/`, `css/`, `scripts/`, `configs/`)
- [ ] **INFRA-02**: Tailscale daemon is permitted to issue TLS certs to the Caddy user (`TS_PERMIT_CERT_UID=caddy` in `/etc/default/tailscaled`)
- [ ] **INFRA-03**: Caddy serves the blog over HTTPS using a Tailscale-issued `*.ts.net` TLS certificate (`get_certificate tailscale`)
...
- [ ] **REND-01**: Caddy rewrites article URLs to an HTML template for Markdown-to-HTML rendering
...
- [ ] **REND-06**: Caddy serves raw Markdown to clients sending `Accept: text/markdown` without `Accept: text/html`
- [ ] **REND-07**: HTTP responses include `Vary: Accept` header when content negotiation is active
...
- [ ] **STYL-04**: Templates directory is blocked from direct browser access (403)
